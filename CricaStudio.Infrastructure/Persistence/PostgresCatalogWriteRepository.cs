...
public sealed class PostgresCatalogWriteRepository(string connectionString) : ICatalogWriteRepository
{
    ...
    public async Task<ShopProduct> SaveShopProductAsync(ShopProduct product, CancellationToken cancellationToken)
    {
        ...
        const string sql = """INSERT INTO cricastudio.shop_products (id,type_id,name,description,full_description,price_mode,price,is_demo,characteristics,personalization,status,sort_order) VALUES (@id,@typeId,@name,@description,@fullDescription,@priceMode,@price,@demo,@characteristics::jsonb,@personalization::jsonb,@status,@order) ON CONFLICT (id) DO UPDATE SET type_id=EXCLUDED.type_id,name=EXCLUDED.name,description=EXCLUDED.description,full_description=EXCLUDED.full_description,price_mode=EXCLUDED.price_mode,price=EXCLUDED.price,is_demo=EXCLUDED.is_demo,characteristics=EXCLUDED.characteristics,personalization=EXCLUDED.personalization,status=EXCLUDED.status,sort_order=EXCLUDED.sort_order;""";
    ...
    {
        var id = product.Id == Guid.Empty ? Guid.NewGuid() : product.Id;
        const string sql = """INSERT INTO cricastudio.affiliate_products (id,type_id,name,description,platform,image_url,affiliate_url,seller,is_demo_listing,status,sort_order) VALUES (@id,@typeId,@name,@description,@platform,@image,@url,@seller,@demo,@status,@order) ON CONFLICT (id) DO UPDATE SET type_id=EXCLUDED.type_id,name=EXCLUDED.name,description=EXCLUDED.description,platform=EXCLUDED.platform,image_url=EXCLUDED.image_url,affiliate_url=EXCLUDED.affiliate_url,seller=EXCLUDED.seller,is_demo_listing=EXCLUDED.is_demo_listing,status=EXCLUDED.status,sort_order=EXCLUDED.sort_order;""";
