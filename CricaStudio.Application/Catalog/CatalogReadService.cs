...
public sealed class CatalogReadService(ICatalogReadRepository repository)
{
    ...
    public Task<CatalogPage<AffiliateProduct>> GetPublishedAffiliateProductPageAsync(int page, int pageSize, string? query, string? platform, bool featuredOnly, CancellationToken cancellationToken) =>
        repository.GetPublishedAffiliateProductPageAsync(page, pageSize, query, platform, featuredOnly, cancellationToken);
